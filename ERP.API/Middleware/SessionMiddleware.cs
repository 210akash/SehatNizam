...
namespace ERP.API.Middleware
{
    ...
    public class SessionMiddleware
    {
            ...
            SessionProvider sessionProvider)
        {
            ...
            var SelectedWarehouseId = context.User.FindFirstValue("SelectedWarehouseId");
            var RetailUserShopId = context.User.FindFirstValue("RetailUserShopId");
            ...
            var dealerShipId = context.User.FindFirstValue("DealerShipId");
            var zoneId = context.User.FindFirstValue("ZoneId");
            var territoryId = context.User.FindFirstValue("TerritoryId");
            ...
            {
                sessionProvider.Session.DealershipId = Convert.ToInt64(dealerShipId);
            }

            if (!string.IsNullOrEmpty(territoryId))
            {
                sessionProvider.Session.TerritoryId = Convert.ToInt64(territoryId);
            }

            if (!string.IsNullOrEmpty(zoneId))
            {
                sessionProvider.Session.ZoneId = Convert.ToInt64(zoneId);
            }

            if (!string.IsNullOrEmpty(SelectedWarehouseId))
            {
                sessionProvider.Session.SelectedWarehouseId = Convert.ToInt64(SelectedWarehouseId);
            }

            if (!string.IsNullOrEmpty(RetailUserShopId))
            {
                sessionProvider.Session.RetailUserShopId = Convert.ToInt64(RetailUserShopId);
            }
