...
namespace ERP.Mediator.Mediator.Account.Handler
{
    public class SaveAccountHandler : IRequestHandler<SaveAccountCommand, long>
    {
        ...
        {
            var AccountType = await unitOfWork.Repository<Entities.Models.AccountType>().GetFirstAsNoTrackingAsync(x => x.IsActive == true && x.Id == request.AccountTypeId);
            var Account = await unitOfWork.Repository<Entities.Models.Account>().GetFirstAsNoTrackingAsync(x => x.IsActive == true && x.Id == request.Id);
            ...
            if (checkDuplicate.Count() == 0)
            {
                ...
                {
                    string _AccountCode = "";
                    if (await unitOfWork.Repository<Entities.Models.Account>().GetExistsAsync(y => y.IsActive == true && y.CompanyId == sessionProvider.Session.CompanyId && y.AccountTypeId == request.AccountTypeId))
                    {
