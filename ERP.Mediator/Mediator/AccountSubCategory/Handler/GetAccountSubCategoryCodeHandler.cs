...
namespace ERP.Mediator.Mediator.AccountSubCategory.Handler
{
    public class GetAccountSubCategoryCodeHandler : IRequestHandler<GetAccountSubCategoryCodeQuery, string>
    {
        ...
        {
            var Category = await unitOfWork.Repository<Entities.Models.AccountCategory>().GetFirstAsNoTrackingAsync(x => x.Id == request.AccountCategoryId && x.IsActive == true);
            string _AccountSubCategoryCode = "";
            if (await unitOfWork.Repository<Entities.Models.AccountSubCategory>().GetExistsAsync(y =>  y.CompanyId == sessionProvider.Session.CompanyId && y.AccountCategoryId == request.AccountCategoryId && y.Id != request.Id && y.IsActive == true))
            {
