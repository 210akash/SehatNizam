...
namespace ERP.Mediator.Mediator.AccountSubCategory.Handler
{
    public class SaveAccountSubCategoryHandler : IRequestHandler<SaveAccountSubCategoryCommand, long>
    {
        ...
        {
            var Category = await unitOfWork.Repository<Entities.Models.AccountCategory>().GetFirstAsNoTrackingAsync(x => x.IsActive == true && x.Id == request.AccountCategoryId);
            var AccountSubCategory = await unitOfWork.Repository<Entities.Models.AccountSubCategory>().GetFirstAsNoTrackingAsync(x => x.IsActive == true &&  x.Id == request.Id);
            ...
            if (checkDuplicate.Count() == 0)
            {
                if (AccountSubCategory == null)
                {
                    ...
                    if (await unitOfWork.Repository<Entities.Models.AccountSubCategory>().GetExistsAsync(y => y.IsActive == true && y.CompanyId == sessionProvider.Session.CompanyId && y.AccountCategoryId == request.AccountCategoryId))
                    {
