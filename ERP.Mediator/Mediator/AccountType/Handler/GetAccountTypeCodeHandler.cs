...
namespace ERP.Mediator.Mediator.AccountType.Handler
{
    public class GetAccountTypeCodeHandler : IRequestHandler<GetAccountTypeCodeQuery, string>
    {
        ...
        {
            var AccountSubCategory = await unitOfWork.Repository<Entities.Models.AccountSubCategory>().GetFirstAsNoTrackingAsync(x => x.Id == request.AccountSubCategoryId && x.IsActive == true);
            string _AccountTypeCode = "";
            if (await unitOfWork.Repository<Entities.Models.AccountType>().GetExistsAsync(y =>  y.IsActive == true && y.CompanyId == sessionProvider.Session.CompanyId && y.AccountSubCategoryId == request.AccountSubCategoryId && y.Id != request.Id))
            {
