...
namespace ERP.Mediator.Mediator.AccountType.Handler
{
    public class SaveAccountTypeHandler : IRequestHandler<SaveAccountTypeCommand, long>
    {
        ...
        {
            var AccountSubCategory = await unitOfWork.Repository<Entities.Models.AccountSubCategory>().GetFirstAsNoTrackingAsync(x => x.IsActive == true && x.Id == request.AccountSubCategoryId);
            var AccountType = await unitOfWork.Repository<Entities.Models.AccountType>().GetFirstAsNoTrackingAsync(x => x.IsActive == true && x.Id == request.Id);
            ...
            if (checkDuplicate.Count() == 0)
            {
                if (AccountType == null)
                {
                    ...
                    if (await unitOfWork.Repository<Entities.Models.AccountType>().GetExistsAsync(y => y.IsActive == true && y.CompanyId == sessionProvider.Session.CompanyId && y.AccountSubCategoryId == request.AccountSubCategoryId))
                    {
