...
{
    public class ProcessPurchaseInvoiceHandler : IRequestHandler<ProcessPurchaseInvoiceQuery, long>
    {
        ...
        public ProcessPurchaseInvoiceHandler(IUnitOfWork unitOfWork, SessionProvider sessionProvider, IMapper mapper)
        {
        ...
        public async Task<long> Handle(ProcessPurchaseInvoiceQuery request, CancellationToken cancellationToken)
        {
            var GRNUpdate = await unitOfWork.Repository<Entities.Models.GRN>().GetFirstAsNoTrackingAsync(y => y.Id == request.Id);

            if(GRNUpdate != null)
            {
                if(GRNUpdate.InvoiceStatusId == 1)
                {
                    GRNUpdate.InvoiceStatusId = 2;
                    GRNUpdate.InvoiceProcessedDate = DateTime.Now;
                    GRNUpdate.InvoiceProcessedById = sessionProvider.Session.LoggedInUserId;
                }
                else if (GRNUpdate.InvoiceStatusId == 2)
                {
                    GRNUpdate.InvoiceStatusId = 6;
                    GRNUpdate.InvoiceAuditVerifiedDate = DateTime.Now;
                    GRNUpdate.InvoiceAuditVerifiedById= sessionProvider.Session.LoggedInUserId;
                }
                else if (GRNUpdate.InvoiceStatusId == 6)
                {
