...
{
    public class GetDispatchCountHandler : IRequestHandler<GetDispatchCountQuery, Tuple<long, long, long, long>>
    {
        ...
        public async Task<Tuple<long, long, long, long>> Handle(GetDispatchCountQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                predicate = x => x.IsActive == true 
                          && x.ProjectId == this.sessionProvider.Session.SelectedWarehouseId && x.CreatedDate >= request.FDate.Value
                          && x.CreatedDate <= request.TDate.Value.AddDays(1).AddTicks(-1)
                          && (request.DealershipId == 0 || x.DispatchOrder.Any(y => y.Order.DealershipId == request.DealershipId))
                          && (request.OrderId == "" || x.DispatchOrder.Any(y => y.Order.Id.ToString() == request.OrderId))
                          && (request.Code == "" || x.Code.ToLower().Contains(request.Code));
            ...
            var entity = unitOfWork.Repository<Entities.Models.Dispatch>().GetPagingWhereAsNoTrackingAsync(predicate, null, null, null, null, null);
            int Created = 0;
            if (roles.Contains("Purchase Manager"))
            {
