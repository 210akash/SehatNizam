...
{
    public class GetKCItemsByPriceGroupHandler : IRequestHandler<GetKCItemsByPriceGroupQuery, List<GetItem>>
    {
        ...
        public GetKCItemsByPriceGroupHandler(IUnitOfWork unitOfWork, IMapper mapper, SessionProvider sessionProvider, IUnitOfWorkDapper unitOfWorkDapper)
        {
        ...
        public async Task<List<GetItem>> Handle(GetKCItemsByPriceGroupQuery request, CancellationToken cancellationToken)
        {
            ...
            var priceDetails = await unitOfWork.Repository<PriceGroupDetails>().GetAsync(x => x.PriceGroupId == request.PriceGroupId && productIds.Contains(x.ItemId) && x.IsActive == true && x.IsDelete == false);

            if (priceDetails == null || priceDetails.Count() == 0)
            {
                throw new InvalidOperationException("Price Details Missing Against selected Price Group");
            }
