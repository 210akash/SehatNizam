...
namespace ERP.Mediator.Mediator.LabOrder.Handler
{
    public class GetAllLabOrderHandler : IRequestHandler<GetAllLabOrderQuery, Tuple<IEnumerable<GetLabOrder>, long>>
    {
        ...
        public async Task<Tuple<IEnumerable<GetLabOrder>, long>> Handle(GetAllLabOrderQuery request, CancellationToken cancellationToken)
        {
            ...
            Expression<Func<Entities.Models.LabOrder, object>>[] includes = { x => x.Status, x => x.LabOrderType , x => x.Appointment, x => x.Appointment.Patient, x => x.Appointment.Department, x => x.Appointment.Department.Company, x => x.Appointment.Referrer, x => x.LabOrderType.Service, x => x.LabOrderType.Variables , x => x.LabResult };
