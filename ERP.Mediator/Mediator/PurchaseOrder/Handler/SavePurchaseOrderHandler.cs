        ...
        async Task<long> IRequestHandler<SavePurchaseOrderCommand, long>.Handle(SavePurchaseOrderCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (PurchaseOrder == null)
                {
                    ...
                    if (await unitOfWork.Repository<Entities.Models.PurchaseOrder>().GetExistsAsync(y=>y.IsActive))
                    {
                ...
                else
                {
                    ...
                    _PurchaseOrder.CompanyId = PurchaseOrder.CompanyId;
                    ...
                    foreach (var PurchaseOrderD in detailupdate)
                    {
                        if (PurchaseOrderD.Id != 0)
                        {
                            ...
                            updatedetail.UnitRate = PurchaseOrderD.UnitRate;
                            updatedetail.Value = PurchaseOrderD.Value;
                            updatedetail.GST = PurchaseOrderD.GST;
                            updatedetail.FED = PurchaseOrderD.FED;
