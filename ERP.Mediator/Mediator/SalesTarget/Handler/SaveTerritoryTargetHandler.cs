...
{
    public class SaveTerritoryTargetHandler : IRequestHandler<SaveTerritoryTargetCommand, long>
    {
        ...
        public SaveTerritoryTargetHandler(IMapper mapper, IUnitOfWork unitOfWork, SessionProvider sessionProvider, IBlobService blobService)
        {
            this.mapper = mapper;
        ...
        async Task<long> IRequestHandler<SaveTerritoryTargetCommand, long>.Handle(SaveTerritoryTargetCommand request, CancellationToken cancellationToken)
        {
            foreach (var item in request.TerritoriesTargetList)
            {
                var salesTarget = await unitOfWork.Repository<Entities.Models.SalesTarget>().GetFirstAsNoTrackingAsync(x => x.TargetMonth.Month == request.TargetMonth.Month);
                if (salesTarget == null)
                {
                    var _salesTarget = mapper.Map<Entities.Models.SalesTarget>(item);
                    //_salesTarget.ZoneId = request.ZoneId;
                ...
                {
                    var _salesTarget = mapper.Map<Entities.Models.SalesTarget>(item);
                    //_salesTarget.ZoneId = salesTarget.ZoneId;
