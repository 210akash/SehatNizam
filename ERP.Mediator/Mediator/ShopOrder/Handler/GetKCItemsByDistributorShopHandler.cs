...
{
    public class GetKCItemsByDistributorShopHandler : IRequestHandler<GetKCItemsByDistributorShopQuery, List<GetItemStock>>
    {
        ...
        public GetKCItemsByDistributorShopHandler(IUnitOfWork unitOfWork, IMapper mapper, SessionProvider sessionProvider, IUnitOfWorkDapper unitOfWorkDapper)
        {
        ...
        public async Task<List<GetItemStock>> Handle(GetKCItemsByDistributorShopQuery request, CancellationToken cancellationToken)
        {
            ...
            //if (products != null)
            //{
            //    var territoryId = unitOfWork.Repository<Entities.Models.Dealership>().GetFirstAsNoTrackingAsync(x => x.Id == request.DistributorId).Result.TerritoryId;
            //    // Fetch all relevant Dealer Stocks and Shop Orders in one go
            //    var _dealerOrders = await unitOfWork.Repository<DispatchDetail>().GetAsync(
            //        y => y.OrderItem.Order.DealershipId == request.DistributorId && y.IsActive == true && y.OrderItem.IsActive == true && y.IsDelete == false && y.DispatchOrder.Dispatch.IsActive && y.DispatchOrder.StatusId == (long)OrderStatusEnum.OrderReceived &&
            //             products.Select(p => p.Id).Contains(y.OrderItem.ItemId),
            //        null,
            //        null,
            //        "OrderItem,OrderItem.Order,OrderItem.Order.Dealership,DispatchOrder,DispatchOrder.Dispatch"
            //    );

            //    var _shopOrders = await unitOfWork.Repository<RetailOrderItems>().GetAsync(
            //        y => y.IsActive &&
            //             !y.IsDelete &&
            ...
            //    var _returnOrders = await unitOfWork.Repository<ShopOrderReturnDetail>().GetAsync(
            //     y => y.IsActive &&
            //          !y.IsDelete &&
            //          y.ShopOrderReturn.StatusId == 3 &&
            //          y.OrderItems.Order.ShopId == sessionProvider.Session.RetailUserShopId &&
            //          y.OrderItems.Order.OrderStatusId == (long)OrderStatusEnum.OrderReceived &&
            //          products.Select(p => p.Id).Contains(y.OrderItems.ItemId),
            //     null,
            //     null,
            //     "ShopOrderReturn,OrderItems,OrderItems.Order"
            // );

            //    // Group the orders by product
            //    var groupeddealerOrders = _dealerOrders.GroupBy(o => o.OrderItem.ItemId);
            //    var groupedshopOrders = _shopOrders.GroupBy(o => o.ItemId);
            //    var groupedreturnOrders = _returnOrders.GroupBy(o => o.OrderItems.ItemId);

            //    foreach (var item in products)
            //    {
            //        // Get all orders for this product
            //        var ordersForProduct = groupeddealerOrders.FirstOrDefault(g => g.Key == item.Id);
            //        var ShopordersForProduct = groupedshopOrders.FirstOrDefault(g => g.Key == item.Id);
            //        var ReturnordersForProduct = groupedreturnOrders.FirstOrDefault(g => g.Key == item.Id);

            //        if (ordersForProduct != null)
            //        {
            //            // Dealer Orders
            //            var totalDealerQty = (int)ordersForProduct.Where(y =>
            //                y.OrderItem.ItemId == item.Id
            //            ).Sum(s => s.Quantity);

            //            // Shop Orders
            //            var shopOrders = ShopordersForProduct.Where(y =>
            //               y.ItemId == item.Id
            //            );
            ...
            //                   y.OrderItems.ItemId == item.Id
            //                ).Sum(y => y.Quantity);

            //            }

            //            // Left Quantity
            //            item.LeftQuantity = totalDealerQty + returnOrders - totalDealerSoldQty;
            //            Console.WriteLine("item : " + item.Name, ", Total In : " + totalDealerQty + ", Sold : " + totalDealerSoldQty);
            //            System.Diagnostics.Debug.WriteLine($"item: {item.Name}, Total In: {totalDealerQty}, Sold: {totalDealerSoldQty}");
            ...
            var distributorPriceGroups = await unitOfWork.Repository<DistributorPriceGroup>().GetAsync(x => x.DealershipId == request.DistributorId && x.IsActive == true && x.IsDelete == false);
