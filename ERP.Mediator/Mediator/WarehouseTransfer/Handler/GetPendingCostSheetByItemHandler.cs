        ...
        public async Task<List<GetCostSheet>> Handle1(GetPendingCostSheetByItemQuery request, CancellationToken cancellationToken)
        {
           ...
           .GetAsync(
               cs => cs.IsActive &&
                     ...
                     "CostSheet,WarehouseTransfer,InspectionDetail,InspectionDetail.IGPDetail,InspectionDetail.IGPDetail,InspectionDetail.IGPDetail.PurchaseOrderDetail,InspectionDetail.IGPDetail.PurchaseOrderDetail.PurchaseDemandDetail"
           );

            var TransferFromCostSheets = await unitOfWork
                 .Repository<WarehouseTransferDetail>()
                 .GetAsync(
                     cs => cs.IsActive &&
                           cs.ItemId == request.ItemId &&
                           cs.WarehouseTransfer.TransferToId == request.ProjectId &&
                           cs.WarehouseTransfer.StatusId == (long)OrderStatusEnum.Approved,
                           null,
                           orderByDec: q => q.OrderByDescending(cs => cs.CreatedDate),
                           "CostSheet,WarehouseTransfer"
                 );

            var TransferToCostSheets = await unitOfWork
               .Repository<WarehouseTransferDetail>()
               .GetAsync(
                   cs => cs.IsActive &&
                         cs.ItemId == request.ItemId &&
