...
namespace ERP.Services.Implementation
{
    ...
    public class BlobService : IBlobService
    {
        ...
        private readonly string _wwwrootPath = "/var/www/publish/ERP/API/wwwroot/logs/log20240418.txt";
        /// <summary>
        ...
        public async Task<bool> DeleteBlobDataAsync(string fileUrl)
        {
            try
            {
                // Convert fileUrl to local path (if it's a full path just use it directly)
                string fullFolderPath = Path.Combine(Localcontainer, fileUrl);
             
                Uri uriObj = new Uri(fullFolderPath);
                string filePath = uriObj.LocalPath;
            ...
            {
                string logFile = Path.Combine(_wwwrootPath);
                string logMessage =
                    $"{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss} - Error deleting file: {ex.Message}{Environment.NewLine}{ex.StackTrace}{Environment.NewLine}";
        ...
        public async Task<string> UploadBase64FileToBlobAsync(BlobImageUploadModel blobModel, string extension = "")
        {
            if (!string.IsNullOrEmpty(blobModel.File))
            {
                ...
                if (imageBytes.Length > 0)
                {
                    string fileExtension = !string.IsNullOrEmpty(extension)
                        ? extension
                        : blobModel.File.Split(';')[0].Split('/')[1];

                    string fileName = this.GenerateFileName(Path.GetFileNameWithoutExtension(blobModel.FileName)) + $".{fileExtension}";

                    string yearFolder = DateTime.Now.ToString("yyyy");
                    string monthFolder = DateTime.Now.ToString("MM");
                    string fullFolderPath = Path.Combine(Localcontainer, blobModel.FolderName, yearFolder, monthFolder);
                    ...
                    try
                    {
                        using (var imageFile = new FileStream(filePath, FileMode.Create))
                        {
                            await imageFile.WriteAsync(imageBytes, 0, imageBytes.Length);
                            imageFile.Flush();
                            return $"{yearFolder}/{monthFolder}/{fileName}"; // Return relative path
                        }
                    }
                    catch (Exception ex)
                    {
