...
@Component({
    selector: 'app-approve-auditreview',
    templateUrl: './approve-auditreview.component.html',
    styleUrl: './approve-auditreview.component.css',
    standalone: false
})

export class ApproveAuditReviewComponent {
  isLoading = false;
  tAmount: any = 0;
  tDiscount: any = 0;
  tExpense: any = 0;
  ...
  partyAmount: any = 0;
  netAmount: any = 0;
  constructor(private dialog: MatDialog, private notificationsService: NotificationsService, private auditreviewService: AuditReviewService,  @Inject(MAT_DIALOG_DATA) public data: { element: any }) { }
  ...
  async approve() {
    (await this.auditreviewService.approveAuditReview(this.data.element.id)).subscribe({
      next: (data) => {
        if (data == true) {
