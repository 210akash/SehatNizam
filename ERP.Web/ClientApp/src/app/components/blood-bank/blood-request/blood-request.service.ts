...
import { BloodRequestEndPoints } from './blood-request.endpoints';

@Injectable({ providedIn: 'root' })
export class BloodRequestService extends BaseService<any> {
    endPointControllerName = 'BloodRequest';

    constructor(httpClient: HttpClient, private endpoints: BloodRequestEndPoints) {
    ...
    getLog(id: number) {
        return this.get('?id=' + id, this.endPointControllerName + this.endpoints.getLog).pipe(map((data: any) => data));
    }
