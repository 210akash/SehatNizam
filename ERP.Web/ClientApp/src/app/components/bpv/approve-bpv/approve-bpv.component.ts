...
@Component({
  selector: 'app-approve-bpv',
  templateUrl: './approve-bpv.component.html',
  styleUrl: './approve-bpv.component.css',
  standalone: false
})

export class ApproveBpvComponent {
  isLoading = false;
  ...
  async reject() {
    (await this.transactionService.rejectTransaction(this.data.element.id)).subscribe({
      next: (data: boolean) => {
        if (data == true) {
          this.isLoading = false;
          this.notificationsService.showNotification('Voucher Rejected', 'snack-bar-success');
          this.dialog.closeAll();
        }
      },
      error: (error: string) => {
