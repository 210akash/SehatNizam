...
@Component({
  selector: 'app-add-brv',
  templateUrl: './add-brv.component.html',
  styleUrl: './add-brv.component.css',
  standalone: false
})

export class AddBrvComponent {
  ...
  ngOnInit(): void {
    this.transactionForm = this.formBuilder.group({
      ...
      voucherTypeId: [5, Validators.required],
      status: [''], // Validation
      remarks: [''], // Validation
      chequeNo: [''], // Validation
    ...
    if (element != null) {
      ...
      if (element.transactionDetails && element.transactionDetails.length > 0) {
        ...
        element.transactionDetails.forEach((detail: any) => {
          ...
          const accountIdValidator = detail.isGroup ? Validators.nullValidator : Validators.required;
          const accountGroupIdValidator = detail.isGroup ? Validators.required : Validators.nullValidator;
      
          const detailGroup = this.formBuilder.group({
            id: [detail.id],
            transactionId: [detail.transactionId],
            accountId: [ detail.accountId == 0 ? null : detail.accountId , accountIdValidator],  // Use the correct value based on isGroup
            ...
            accountGroupId: [detail.accountGroupId == 0 ? null : detail.accountGroupId, accountGroupIdValidator],  // Use the correct value for accountGroupId
            accountGroup: [detail.accountGroup ? detail.accountGroup : ''],
            accountGroupName: [detail.accountGroup ? detail.accountGroup.code + ':' +  detail.accountGroup.name : '', Validators.required],
  ...
  SaveData() {
    ...
    let chequeDate = new Date(this.transactionForm.get('chequeDate')?.value);
    _transactionForm['chequeDate'] = chequeDate.toLocaleDateString();
  ...
  getTransactionCode() {
    ...
    this.transactionService.getTransactionCode(VoucherTypeId,VoucherDate).subscribe((data: any) => {
  ...
  async changeDrCr(type:any,index:number) {
    ...
    if(type == 'dr'){
      ...
      detailFormGroup.get('creditAmount')?.setValidators([ Validators.min(0), Validators.max(0)]);
      detailFormGroup.get('creditAmount')?.updateValueAndValidity();
      ...
      detailFormGroup.get('debitAmount')?.clearValidators();
      detailFormGroup.get('debitAmount')?.updateValueAndValidity();
      detailFormGroup.get('debitAmount')?.setValidators([Validators.min(0), Validators.max(0)]);
      detailFormGroup.get('debitAmount')?.updateValueAndValidity();
      detailFormGroup.get('creditAmount')?.clearValidators();
  ...
  onOptionSelectedSearch(event: MatAutocompleteSelectedEvent, index: number): void {
    ...
    getaccountgroup(itemId: string) {
      ...
      onOptionGroupSelected(event: MatAutocompleteSelectedEvent, index: number): void {
        const selectedValue = event.option.value;
        const transactionDetailArray = this.transactionForm.get('transactionDetails') as FormArray;
        ...
        if (duplicateItem) {
          ...
          // Patch the values into the form group
           ...
           detailFormGroup.get('accountGroupName')?.patchValue(selectedValue.code + ' : ' + selectedValue.name);
        ...
        if(checked){
          ...
          currentFormGroup.get('accountGroupId')?.setValidators([Validators.required]);
          currentFormGroup.get('accountGroupId')?.updateValueAndValidity();
          currentFormGroup.get('accountGroupName')?.setValidators([Validators.required]);
          currentFormGroup.get('accountGroupName')?.updateValueAndValidity();
          currentFormGroup.get('accountGroup')?.setValidators([Validators.required]);
