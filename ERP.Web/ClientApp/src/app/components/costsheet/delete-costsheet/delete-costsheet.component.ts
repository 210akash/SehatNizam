...
@Component({
    selector: 'app-delete-costsheet',
    templateUrl: './delete-costsheet.component.html',
    styleUrl: './delete-costsheet.component.css',
    standalone: false
})

export class DeleteCostSheetComponent {
  isLoading = false;
  ...
  async delete() {
    (await this.costsheetService.deleteCostSheet(this.data.element.id)).subscribe({
      next: (data) => {
        if (data == true) {
