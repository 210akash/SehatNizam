...
@Component({
    selector: 'app-process-costsheet',
    templateUrl: './process-costsheet.component.html',
    styleUrl: './process-costsheet.component.css',
    standalone: false
})

export class ProcessCostSheetComponent {
  isLoading = false;
  ...
  advFEDAmt!: number;
  constructor(private dialog: MatDialog, private notificationsService: NotificationsService, private costsheetService: CostSheetService, private constantService: ConstantService, @Inject(MAT_DIALOG_DATA) public data: { element: any }) { }
  ...
  async process() {
    (await this.costsheetService.processCostSheet(this.data.element.id)).subscribe({
      next: (data: boolean) => {
        if (data == true) {
