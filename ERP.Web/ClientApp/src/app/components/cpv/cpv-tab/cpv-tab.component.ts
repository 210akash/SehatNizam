...
import { CpvListComponent } from '../cpv-list/cpv-list.component';
import { AddCpvComponent } from '../add-cpv/add-cpv.component';

@Component({
  selector: 'app-cpv-tab',
  templateUrl: './cpv-tab.component.html',
  styleUrls: ['./cpv-tab.component.css'],
  standalone: false,
})
export class CpvTabComponent implements OnInit {
  ...
  @ViewChild(CpvListComponent) transactionListComponent!: CpvListComponent;
  @Output() getTransactionCount1: EventEmitter<void> = new EventEmitter<void>();
  async ngOnInit() {
    ...
    this.TransactionFilterForm.get('fdate')?.patchValue(
      this.constantService.formatDate(startDate)
    );
    this.TransactionFilterForm.get('tdate')?.patchValue(
      this.constantService.formatDate(endDate)
    );
  ...
  async fillGridCount(statusId: any) {
    ...
    _TransactionFilterForm = Object.assign(
      _TransactionFilterForm,
  ...
  async getTransactionCount() {
    this.transactionService
      .getTransactionCount(this.lObjLeadsFilterForm)
      .subscribe({
        next: async (data: any) => {
  ...
  async openTransactionDialog(element: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      this.getTransactionCount1.emit(); // Change to the index of the tab you want to select
      this.changeTabs(0);
