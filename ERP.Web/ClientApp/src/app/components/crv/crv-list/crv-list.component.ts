...
import { AddCrvComponent } from '../add-crv/add-crv.component';
import { ViewCrvComponent } from '../view-crv/view-crv.component';
import { DeleteCrvComponent } from '../delete-crv/delete-crv.component';
import { ProcessCrvComponent } from '../process-crv/process-crv.component';
import { PrintCrvComponent } from '../print-crv/print-crv.component';
import { ApproveCrvComponent } from '../approve-crv/approve-crv.component';

@Component({
  selector: 'app-crv-list',
  templateUrl: './crv-list.component.html',
  styleUrls: ['./crv-list.component.css'],
  standalone: false
})

export class CrvListComponent {
  [x: string]: any;
  ...
  async bindData(transactionFilterForm: any, currenttab: number, isFromParent: boolean): Promise<void> {
  ...
  openTransactionDialog(element: any) {
    const dialogRef = this.dialog.open(AddCrvComponent, {
  ...
  viewTransactionDialog(element: any): void {
    this.dialog.open(ViewCrvComponent, {
  ...
  deleteTransactionDialog(element: any) {
    const dialogRef = this.dialog.open(DeleteCrvComponent, {
      panelClass: 'cstm_width_1300',
      maxHeight: '90vh',
      data: {
        element: element,
  ...
  processTransactionDialog(element: any) {
    const dialogRef = this.dialog.open(ProcessCrvComponent, {
      panelClass: 'cstm_width_1300',
      maxHeight: '90vh',
      data: {
        element: element,
  ...
  approveTransactionDialog(element: any) {
    const dialogRef = this.dialog.open(ApproveCrvComponent, {
      panelClass: 'cstm_width_1300',
      maxHeight: '90vh',
      data: {
        element: element,
  ...
  printTransactionDialog(element: any) {
    const dialogRef = this.dialog.open(PrintCrvComponent, {
