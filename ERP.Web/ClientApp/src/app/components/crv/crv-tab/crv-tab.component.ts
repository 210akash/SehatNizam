...
import { CrvListComponent } from '../crv-list/crv-list.component';
import { AddCrvComponent } from '../add-crv/add-crv.component';

@Component({
  selector: 'app-crv-tab',
  templateUrl: './crv-tab.component.html',
  styleUrls: ['./crv-tab.component.css'],
  standalone: false
})

export class CrvTabComponent implements OnInit {
  ...
  @ViewChild(CrvListComponent) transactionListComponent!: CrvListComponent;
  ...
  async changeTab(event: MatTabChangeEvent) {
    ...
    await this.transactionListComponent.bindData(this.lObjLeadsFilterForm, this.selected.value, true);
  }
  ...
  changeTabs(index: number) {
    ...
    this.transactionListComponent.bindData(this.lObjLeadsFilterForm, this.selected.value, true);
    this.getTransactionCount1.emit();
  }
