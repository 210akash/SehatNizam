...
export class EmployeeTypeListComponent {
  EmployeeTypeFilterForm!: FormGroup;
  isLoading = false;
  currentPage = 0;
  pageSize = 0;
  ...
  async ngOnInit(): Promise<void> {
    this.EmployeeTypeFilterForm = this.formBuilder.group({});
  ...
  async bindData(): Promise<void> {
    ...
    const _EmployeeTypeFilterForm = {
      ...this.EmployeeTypeFilterForm.value,
      PagingData: pagingData
    };

    // Call the service method and subscribe with the observer
    this.employeeTypeService.getAllEmployeeTypes(_EmployeeTypeFilterForm).subscribe({
      next: (data: any) => {
  ...
  openEmployeeTypeDialog(element: any) {
    const dialogRef = this.dialog.open(AddEmployeeTypeComponent, {
  ...
  viewEmployeeTypeDialog(element: any): void {
    this.dialog.open(ViewEmployeeTypeComponent, {
  ...
  deleteEmployeeTypeDialog(element: any) {
    const dialogRef = this.dialog.open(DeleteEmployeeTypeComponent, {
      panelClass: 'cstm_width_500',
      height: 'auto',
      data: {
        element: element,
