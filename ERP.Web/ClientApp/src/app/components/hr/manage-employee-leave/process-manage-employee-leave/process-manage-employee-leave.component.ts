...
@Component({
  selector: 'app-process-manage-employee-leave',
  templateUrl: './process-manage-employee-leave.component.html',
  styleUrl: './process-manage-employee-leave.component.css',
  standalone: false
})

export class ProcessManageEmployeeLeaveComponent {
...
this.getEmployeeLeaveBalance(this.data.element.employeeId);
   ...
   Approve() {
    this.employeeLeaveService.approveEmployeeLeave(this.processEmployeeLeaveForm.value).subscribe({
      next: (data) => {
        if (data == 200) {
