...
constructor(
    ...
    private notificationsService: NotificationsService, private rosterService: RosterService, private constantService: ConstantService,private dialogRef: MatDialogRef<ProcessRosterComponent>
  ) {
    ...
    (await this.rosterService.processRoster(this.data.element.id)).subscribe({
      next: (data: boolean) => {
        ...
        if (data === true) {
          ...
          this.notificationsService.showNotification('Unable to process roster', 'snack-bar-danger');
        }
      },
      error: (error: string) => {
        console.log(error);
