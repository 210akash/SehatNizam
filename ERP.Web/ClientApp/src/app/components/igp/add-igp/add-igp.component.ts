...
import { IGPTypeService } from '../../igptype/igptype.service';
...
export class AddIGPComponent {
  ...
  constructor(private igpTypeService: IGPTypeService,private dialog: MatDialog, private notificationsService: NotificationsService, private formBuilder: FormBuilder, private iGPService: IGPService, private purchaseOrderService: PurchaseOrderService, private departmentService: DepartmentService, private constantService: ConstantService, @Inject(MAT_DIALOG_DATA) public data: { element: any }) { }
  ...
  ngOnInit(): void {
    this.iGPForm = this.formBuilder.group({
      ...
      projectId: [0],
      igpTypeId: [1, Validators.required],
  ...
  getPOData() {
    ...
    return this.purchaseOrderList.find(x => x.id === purchaseOrderId);
  }
  ...
  async getPurchaseOrdersDetails(): Promise<void> {
    const purchaseOrderId = this.iGPForm.get('purchaseOrderId')?.value;
    const IgpId = this.iGPForm.get('id')?.value;

    try {
      const data = await (await this.iGPService.getPendingPOItems(purchaseOrderId,IgpId)).toPromise();
      this.itemList = data;
      ...
      if (this.itemList && this.itemList.length > 0) {
         this.itemList.forEach((detail: any) => {
          ...
          const detailGroup = this.formBuilder.group({
            id: [0],
            iGPId: [this.iGPForm.get('id')?.value],
            purchaseOrderDetailId: [detail.id, Validators.required],
            itemId: [detail?.purchaseDemandDetail?.itemId, Validators.required],
            orderedQuantity: [detail.quantity, Validators.required],
    ...
    async getIGPType() {
    let _IGPTypeFilter: any = {};
    (await this.igpTypeService.getAllIGPType(_IGPTypeFilter)).subscribe((data: any) => {
     this.igpTypeList = data.item1;
    });
  }
