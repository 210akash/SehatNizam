...
export class IGPService extends BaseService<any> {
    ...
    async getPendingPOItems(purchaseOrderId: number,iGPId :number) {
        return this.get('?purchaseOrderId=' + purchaseOrderId + '&iGPId=' + iGPId , this.endPointControllerName + this.iGPEndPoints.getPendingPOItems)
            .pipe();
    }
