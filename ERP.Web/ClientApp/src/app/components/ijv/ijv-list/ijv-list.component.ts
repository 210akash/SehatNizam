...
import { ViewIJVComponent } from '../view-ijv/view-ijv.component';
import { PrintIJVComponent } from '../print-ijv/print-ijv.component';

@Component({
  selector: 'app-ijv-list',
  templateUrl: './ijv-list.component.html',
  styleUrls: ['./ijv-list.component.css'],
  standalone: false
})

export class IJVListComponent {
  [x: string]: any;
  ...
  async bindData(ijvFilterForm: any, currenttab: number, isFromParent: boolean): Promise<void> {
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      this.isLoading = true;
      this.TransactionFilterForm = ijvFilterForm;
      ...
      ijvFilterForm["PagingData"] = pagingData;
      let fdate = new Date(ijvFilterForm.fdate);
      let tdate = new Date(ijvFilterForm.tdate);

      ijvFilterForm['fdate'] = fdate.toLocaleDateString();
      ijvFilterForm['tdate'] = tdate.toLocaleDateString();

      // Call the service method and subscribe with the observer

      (await this.sjvService.getAllTransactions(ijvFilterForm)).subscribe({
        next: (data: any) => {
  ...
  viewTransactionDialog(element: any): void {
    this.dialog.open(ViewIJVComponent, {
  ...
  printTransactionDialog(element: any) {
    const dialogRef = this.dialog.open(PrintIJVComponent, {
