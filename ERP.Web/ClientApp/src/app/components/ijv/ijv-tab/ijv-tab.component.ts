...
import { IJVListComponent } from '../ijv-list/ijv-list.component';

@Component({
  selector: 'app-ijv-tab',
  templateUrl: './ijv-tab.component.html',
  styleUrls: ['./ijv-tab.component.css'],
  standalone: false
})

export class IJVTabComponent implements OnInit {
  ...
  constructor(private sjvService: TransactionService, private dialog: MatDialog, private constantService: ConstantService, private formBuilder: FormBuilder) { }
  @ViewChild(IJVListComponent) ijvListComponent!: IJVListComponent;
  ...
  async changeTab(event: MatTabChangeEvent) {
    ...
    await this.ijvListComponent.bindData(this.lObjLeadsFilterForm, this.selected.value, true);
  }
  ...
  async getTransactionCount() {
    (this.sjvService.getTransactionCount(this.lObjLeadsFilterForm)).subscribe(
      {
        next: async (data: any) => {
