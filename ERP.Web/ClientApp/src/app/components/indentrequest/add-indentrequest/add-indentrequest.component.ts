...
export class AddIndentrequestComponent {
  ...
  addIndentRequestDetail(index: number) {
    const newDetailGroup = this.formBuilder.group({
      ...
      required: [0.000, [Validators.required, Validators.min(0.000)]], // Validation
  ...
  LoadData(element: any) {
    if (element != null) {
      ...
      if (element.indentRequestDetail && element.indentRequestDetail.length > 0) {
       ...
       this.storeList =  this.projectList.find((option: { id: string; }) => option.id === element.projectId).projectStore;
    }
    else {
      this.addIndentRequestDetail(0);
  ...
  getItemList(event: any) {
    var storeId  = this.indentrequestForm.get('storeId')?.value;
    if(storeId != 0 || storeId != ''){
      ...
      this.notificationsService.showNotification('Please Select Store', 'snack-bar-danger');
    }
  }
...
async getprojectList(): Promise<void> {
  ...
  const data = await firstValueFrom(this.projectService.getAllProjects(_storeFilter));
  ...
  changeProject() {
    const projectId = this.indentrequestForm.get('projectId')?.value;
    this.storeList =  this.projectList.find((option: { id: string; }) => option.id === projectId).projectStore;
  }
