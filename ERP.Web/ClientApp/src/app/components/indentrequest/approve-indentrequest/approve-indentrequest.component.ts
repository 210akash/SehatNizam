...
@Component({
    selector: 'app-approve-indentrequest',
    templateUrl: './approve-indentrequest.component.html',
    styleUrl: './approve-indentrequest.component.css',
    standalone: false
})

export class ApproveIndentrequestComponent {
  ...
  async approve() {
    (await this.indentrequestService.approveIndentrequest(this.data.element.id)).subscribe({
      next: (data) => {
        if (data == true) {
