...
@Component({
  selector: 'app-print-issuance',
  templateUrl: './print-issuance.component.html',
  styleUrl: './print-issuance.component.css',
  standalone: false
})

export class PrintIssuanceComponent {
  ...
  getVendorDetail(detailId: number, vendorId: number) {
    ...
    const details = this.data.element.issuanceDetail[detailId];

    if (details && details.issuanceVendor) {
      // Filter to include only items where vendorId matches the given vendorId
      const filteredVendors = details.issuanceVendor.filter(
