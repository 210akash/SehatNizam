...
import { ViewItemtypeComponent } from '../view-itemtype/view-itemtype.component';
...
export class ItemtypeListComponent {
  ...
  async ngOnInit(): Promise<void> {
    this.ItemtypeFilterForm = this.formBuilder.group({
      name: [''],
      subCategoryId: ['']
    });
  ...
  async bindData(): Promise<void> {
    ...
    const _ItemtypeFilterForm = {
      ...this.ItemtypeFilterForm.value,
      PagingData: pagingData
    };

    // Call the service method and subscribe with the observer
    this.itemtypeService.getAllItemtypes(_ItemtypeFilterForm).subscribe({
      next: (data: any) => {
  ...
  openItemtypeDialog(element: any) {
    const dialogRef = this.dialog.open(AddItemtypeComponent, {
      width: '30%',
      height: 'auto',
      data: {
        element: element,
  ...
  viewItemtypeDialog(element: any): void {
    this.dialog.open(ViewItemtypeComponent, {
  ...
  deleteItemtypeDialog(element: any) {
