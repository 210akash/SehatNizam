...
@Component({
  selector: 'app-print-triage',
  templateUrl: './print-triage.component.html',
  styleUrls: ['./print-triage.component.css'],
  standalone: false
})
export class PrintTriageComponent {
  currentUser: any;
  currentDate: any;
  currentTime: any;
  qrCodeUrl = '';
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<PrintTriageComponent>,
  ...
  ngOnInit(): void {
    ...
    const appointmentId = this.getAppointment()?.id ?? this.data?.element?.appointmentId ?? '';
  ...
  getAppointment(): any {
    return this.data?.element?.appointment || {};
  }
  ...
  getHospitalName(): string {
    return this.getDepartment()?.company?.name || '-';
  }

  getHospitalSubtitle(): string {
    const company = this.getDepartment()?.company;
    const parts = [
      company?.address,
  ...
  getDoctorName(): string {
    const doctor = this.getAppointment()?.doctor || this.data?.element?.doctor;
    if (!doctor) {
      return '-';
    }
  ...
  getAppointmentStatus(): string {
    ...
    return appointment?.appointmentStatus?.name
  ...
  getClinicalNarrative(): string {
    ...
    const parts = [
      ...
      element?.allergies ? `Allergies: ${element.allergies}` : '',
      element?.medications ? `Medications: ${element.medications}` : '',
      element?.notes,
      appointment?.reason
  ...
  getVitalValue(key: string): string {
    ...
    switch (key) {
      ...
      case 'heightCm':
        return this.getHeightValue(triage);
      case 'rbs':
  ...
  getTriagePriority(): string {
    return this.data?.element?.triagePriority?.name || '-';
  }

  getSugarType(): string {
    return this.data?.element?.sugarType?.name || '-';
  }

  private getPayment(): any {
    const appointment = this.getAppointment();
    return appointment?.appointmentPayments?.find((item: any) => item.appointmentId === appointment?.id)
      || appointment?.appointmentPayments?.[0]
      || appointment?.appointmentPayment
      || {};
  }

  private getTriage(): any {
    const element = this.data?.element;
    return element?.triage
      || element?.appointment?.triages?.find((item: any) => item.id === element?.id)
      || element
      || {};
  }

  private getHeightValue(triage: any): string {
    const heightCm = triage?.heightCm ?? triage?.heightCM;
    if (heightCm != null && heightCm !== '') {
      return String(heightCm);
    }

    const feet = triage?.heightFeet;
    const inches = triage?.heightInches;
    if (feet || inches) {
