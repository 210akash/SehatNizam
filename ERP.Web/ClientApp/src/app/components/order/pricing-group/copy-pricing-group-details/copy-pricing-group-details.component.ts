...
@Component({
  selector: 'app-copy-pricing-group-details',
  templateUrl: './copy-pricing-group-details.component.html',
  styleUrls: ['./copy-pricing-group-details.component.css'], standalone: false
})

export class CopyPricingGroupDetailsComponent implements OnInit {
  copyPricingGroupDetailsForm!: FormGroup;
  isLoading = false;
  ...
  ngOnInit(): void {
    this.getAllProducts();
    this.copyPricingGroupDetailsForm = this.formBuilder.group({
      id: [0],
  ...
  async saveProductPricingDetails() {
    ...
    for (const productControl of productArray) {
      ...
      if (this.copyPricingGroupDetailsForm.invalid) {
      this.constantService.markFormGroupTouched(this.copyPricingGroupDetailsForm);
      return;
    }

    // Your save logic here, e.g., sending data to API
    this.isLoading = true;
    this.copyPricingGroupDetailsForm.value.id = this.data.element.id;
    var dataabc = this.copyPricingGroupDetailsForm.value
    console.log("Final Data Sent to API:", dataabc);

    (await this.pricingGroupService.copyPriceGroup(this.copyPricingGroupDetailsForm.value)).subscribe(
      {
        next: (data) => {
          if (data.Status == 200) {
            this.notificationsService.showNotification('Price Group Copied Successfully!', 'snack-bar-success');
  ...
  get f() {
    return this.copyPricingGroupDetailsForm.controls;
  }

  get getProductGroupDetails(): FormArray {
    return this.copyPricingGroupDetailsForm.get('getProductGroupDetails') as FormArray;
  }
  ...
  async setEditProducts(data: any) {
    const productArray = this.copyPricingGroupDetailsForm.get('getProductGroupDetails') as FormArray;
