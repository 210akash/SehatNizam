...
import { PricingGroupService } from '../../pricing-group/pricing-group.service';
import { MatAutocompleteSelectedEvent } from '@angular/material/autocomplete';

@Component({
  selector: 'app-create-order-with-pg',
  templateUrl: './create-order-with-pg.component.html',
  styleUrl: './create-order-with-pg.component.css',
  standalone: false
})

export class CreateOrderWithPGComponent implements OnInit {
  ...
  constructor(private pricingGroupService: PricingGroupService, private sanitizer: DomSanitizer, private itemService: ItemService, private dealershipService: DealershipService, private notificationsService: NotificationsService, private dialog: MatDialog, private formBuilder: FormBuilder, private constantService: ConstantService, private orderService: PrimaryOrderService, @Inject(MAT_DIALOG_DATA) public data: { element: any }) { }
  ...
  async getAllPriceGroup() {
    ...
    (await this.pricingGroupService.getAllPricingGroup(_productListFilerForm)).subscribe({
      next: (data) => {
        this.priceGroupList = data.item1;
  ...
  onOptionDealershipSelected(event: MatAutocompleteSelectedEvent): void {
    ...
    this.createOrderForm.get('dealershipName')?.patchValue(selectedValue.name + ' | ' + selectedValue.address);
  ...
  onPriceGroupChange() {
    ...
    this.createOrderForm.get('dealershipAddress')?.patchValue(null);
    this.createOrderForm.get('dealershipId')?.patchValue(null);
  ...
  onDealershipChange() {
    ...
    this.getAllProductsByDistributorIdPriceGroupId();
  }

  async getAllProductsByDistributorIdPriceGroupId() {
    if (this.selectedDealer == undefined) {
      // meanz no distributor selected
      this.createOrderForm.setControl('orderItemsList', this.formBuilder.array([]));
      return;
    }
    let priceGroupId = this.createOrderForm.get('priceGroupId')?.value;
    (await this.itemService.getKCItemsByPriceGroupId(this.selectedDealer.territoryId, priceGroupId)).subscribe({
      next: (data: any) => {
        ...
        if (Array.isArray(data) && data.length > 1 && data[0]?.id) {
          ...
          this.createOrderForm.setControl('orderItemsList', this.formBuilder.array([]));
          this.notificationsService.showNotification('Price Details Missing Against selected Price Group!', 'snack-bar-danger');
        }
      },
      error: (error: any) => {
        this.notificationsService.showNotification('Please Fill the required fields!', 'snack-bar-danger');
