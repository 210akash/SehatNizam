...
@Component({
  ...
  styleUrls: ['./edit-order.component.css'], standalone: false
})
...
export class EditOrderComponent implements OnInit {
  ...
  onProductSelected(event: MatSelectChange, index: number): void {
    const selectedValue = event.value;
    const costSheetDetailArray = this.createOrderForm.get('orderItemsList') as FormArray;
    if (!selectedValue) {
      console.error('Option value is undefined. Ensure mat-option [value] is correctly bound.');
      return;
    }
    ...
    // Get the selected item from the productList using the selectedValue
    // Assuming selectedValue is the itemId or some unique identifier for the product
    ...
    const currentFormGroup = costSheetDetailArray.at(index) as FormGroup;
       const targetItem = this.data.element.orderItems[index];
    if (duplicateItem) {
      ...
      // This will clear all the values of the current form group
        currentFormGroup.get('itemId')?.patchValue(targetItem.item.id);
        currentFormGroup.get('image')?.patchValue(targetItem.item.image);
      ...
      const itemVolume = targetItem.item.volume;
      const selectedVolume = selectedItem.volume;
      const volumeMatches = itemVolume === selectedVolume;
      if (!volumeMatches) {
        this.notificationsService.showNotification('Please select same volumn item.', 'snack-bar-danger');
        currentFormGroup.get('itemId')?.patchValue(targetItem.item.id);
        currentFormGroup.get('image')?.patchValue(targetItem.item.image);
        currentFormGroup.updateValueAndValidity();
        return; // Do nothing if the volume does not match
      }

      const detailFormGroup = costSheetDetailArray.at(index) as FormGroup;
      // Patch the values into the form group
      detailFormGroup.get('itemId')?.patchValue(selectedItem.id);
      detailFormGroup.get('item')?.patchValue(selectedItem);
      detailFormGroup.get('itemName')?.patchValue(selectedItem.name);
      detailFormGroup.get('image')?.patchValue(selectedItem.image);
      detailFormGroup.get('leftQuantity')?.patchValue(selectedItem.stockQty);
  ...
  getitem(itemId: string) {
    return this.productList.find((option: { id: string; }) => option.id === itemId);
  }
