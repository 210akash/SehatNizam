...
@Component({
    selector: 'app-process-retail-order-return',
    templateUrl: './process-retail-order-return.component.html',
    styleUrl: './process-retail-order-return.component.css',
    standalone: false
})

export class ProcessRetailOrderReturnComponent {
  isLoading = false;
  ...
  async process() {
    (await this.retailOrderReturnService.processRetailOrderReturn(this.data.element.id)).subscribe({
      next: (data) => {
        if (data == true) {
