...
@Component({
  ...
  styleUrls: ['./view-zone.component.css'],standalone: false
})

export class ViewZoneComponent implements OnInit {
  viewZoneForm!: FormGroup;
  isLoading = false;
  dataSource!: any;
  roleList: any;
  ...
  ngOnInit(): void {
    this.viewZoneForm = this.formBuilder.group({
      id: [0],
      name: [''],
      description: [''],
      coordinates: ['']
  ...
  LoadData(element: any) {
    this.constantService.LoadData(element, this.viewZoneForm);
  }
  ...
  viewZone(): void {
    ...
    coordinatesList.push({
      typeId: 1,
      coordinates: 'Zone-' + this.data.element.coordinates
    });

    const element = {
      caption: 'View Zone ( ' + this.data.element.name + ')',
