...
import { ViewPJVComponent } from '../view-pjv/view-pjv.component';
import { PrintPJVComponent } from '../print-pjv/print-pjv.component';

@Component({
  selector: 'app-pjv-list',
  templateUrl: './pjv-list.component.html',
  styleUrls: ['./pjv-list.component.css'],
  standalone: false
})

export class PJVListComponent {
  [x: string]: any;
  ...
  async bindData(pjvFilterForm: any, currenttab: number, isFromParent: boolean): Promise<void> {
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      this.isLoading = true;
      this.TransactionFilterForm = pjvFilterForm;
      ...
      pjvFilterForm["PagingData"] = pagingData;
      let fdate = new Date(pjvFilterForm.fdate);
      let tdate = new Date(pjvFilterForm.tdate);

      pjvFilterForm['fdate'] = fdate.toLocaleDateString();
      pjvFilterForm['tdate'] = tdate.toLocaleDateString();

      // Call the service method and subscribe with the observer

      (await this.sjvService.getAllTransactions(pjvFilterForm)).subscribe({
        next: (data: any) => {
  ...
  viewTransactionDialog(element: any): void {
    this.dialog.open(ViewPJVComponent, {
  ...
  printTransactionDialog(element: any) {
    const dialogRef = this.dialog.open(PrintPJVComponent, {
