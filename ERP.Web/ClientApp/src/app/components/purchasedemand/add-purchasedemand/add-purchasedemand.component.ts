...
export class AddPurchaseDemandComponent {
  ...
  addPurchaseDemandDetail(index: number) {
    const newDetailGroup = this.formBuilder.group({
      ...
      itemId: [0, Validators.required], // Validation
      indentRequestDetailId: [0, Validators.required], // Validation
  ...
  getItemList(event: any) {
    ...
    var storeId = this.currentUser.store.id;
    this.itemService.getItemByName(filter, storeId).subscribe((data: any) => {
      this.itemList = data;
    });
  }
  private indentRequestLoaded = false;
  async getItemRequestList() {
    const purchaseDemandId = this.purchasedemandForm.get('id')?.value;
    const indentRequestId = this.purchasedemandForm.get('indentRequestId')?.value;
  
    if (!this.indentRequestLoaded) {
      await this.getindentList();
      this.indentRequestLoaded = true; // Avoid loading again
    }
  
    this.seletedIndentRequest = this.indentRequestList.find((x: any) => x.id === indentRequestId);
  
    const observable = await this.purchasedemandService.getPendingIndentItems(indentRequestId, purchaseDemandId);
      ...
      if (element.purchaseDemandDetail && element.purchaseDemandDetail.length > 0) {
        element.purchaseDemandDetail.forEach((detail: any) => {
          const detailGroup = this.formBuilder.group({
            ...
            request: [detail.request, [Validators.required, Validators.min(1)]],
  ...
  onOptionSelected(event: MatAutocompleteSelectedEvent, index: number): void {
    ...
    if (duplicateItem) {
      ...
      currentFormGroup.get('indentRequestDetailId')?.patchValue(0);
      ...
      detailFormGroup.get('indentRequestDetailId')?.patchValue(selectedValue.id);
  ...
  getitemReq(itemId: string) {
    return this.itemList.find((option: { itemId: string; }) => option.itemId === itemId);
  }
  ...
  async getindentList() {
    this.indentRequestList = await firstValueFrom(
      this.purchasedemandService.getPendingIndentRequest(this.purchasedemandForm.get('indentRequestId')?.value)
    );
  }
  ...
  onOptionSelectedIndent(event: MatSelectChange, index: number): void {
    const selectedValue = event.value;  // Directly get the selected value
    const indentRequestDetailArray = this.purchasedemandForm.get('purchaseDemandDetail') as FormArray;
    ...
    const duplicateItem = indentRequestDetailArray.controls
      ...
      .some((control: AbstractControl) => {
        const formGroup = control as FormGroup;  // Cast AbstractControl to FormGroup
        return formGroup.get('itemId')?.value === selectedValue;  // Check if the itemId already exists
      });
    ...
    if (duplicateItem) {
      ...
      // Get the selected item details from your getitem method
      const selectedItem = this.getitemReq(selectedValue);
      if (!selectedItem) {
        console.error('Selected item not found.');
        return;
      }
      ...
      // Patch the values into the form group
      detailFormGroup.get('itemId')?.patchValue(selectedValue);
      detailFormGroup.get('indentRequestDetailId')?.patchValue(selectedItem.id);
      detailFormGroup.get('item')?.patchValue(selectedItem.item);
