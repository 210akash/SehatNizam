...
@Component({
  selector: 'app-print-purchaseinvoice',
  templateUrl: './print-purchaseinvoice.component.html',
  styleUrl: './print-purchaseinvoice.component.css',
  standalone: false,
})
export class PrintPurchaseInvoiceComponent {
  ...
  getTotalQuantity(): any {
    return this.data.element.grnDetail.reduce(
      (total: number, detail: { received: number }) =>
        total + (detail.received || 0),
      0
    );
  }

  getWHTAmount(): any {
    const amount = this.data.element.grnDetail.reduce((total: number, detail: any) => {
      const received = detail?.received ?? 0;
      const unitRate =
        detail?.inspectionDetail?.igpDetail?.purchaseOrderDetail?.unitRate ?? 0;
      return total + received * unitRate;
    }, 0);

    this.gWHT = Math.round((amount * this.data.element.whtPercentage) / 100);
    return (amount * this.data.element.whtPercentage) / 100;
  }

  getTotalPrice(): any {
    return this.data.element.grnDetail.reduce((total: number, detail: any) => {
      const received = detail?.received ?? 0;
      const unitRate =
        detail?.inspectionDetail?.igpDetail?.purchaseOrderDetail?.unitRate ?? 0;
      return total + received * unitRate;
 ...
 async calculateTotals() {
  ...
  getReceivedTotalAmount(): number {
    const total = this.data.element.grnDetail.reduce(
      (sum: number, detail: any) => {
        const received = detail?.received ?? 0;
        const unitRate =
          detail?.inspectionDetail?.igpDetail?.purchaseOrderDetail?.unitRate ??
          0;
        return sum + received * unitRate;
      },
      0
    );
  ...
  printDocument() {
    ...
    if (printContent) {
      const WindowPrt = window.open(
        '',
        '',
        'left=0,top=0,width=1100,height=1100,toolbar=0,scrollbars=0,status=0'
      );
      if (WindowPrt) {
