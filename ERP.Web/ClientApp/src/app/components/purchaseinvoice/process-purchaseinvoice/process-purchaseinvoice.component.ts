...
@Component({
  selector: 'app-process-purchaseinvoice',
  templateUrl: './process-purchaseinvoice.component.html',
  styleUrl: './process-purchaseinvoice.component.css',
  standalone: false,
})

export class ProcessPurchaseInvoiceComponent {
  ...
  async process() {
    ...
    (
      await this.grnService.processPurchaseInvoice(this.data.element.id, comm)
    ).subscribe({
      next: (data: any) => {
        if (data == 200) {
