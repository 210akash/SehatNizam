...
@Component({
    selector: 'app-cancel-purchaseorder',
    templateUrl: './cancel-purchaseorder.component.html',
    styleUrl: './cancel-purchaseorder.component.css',
    standalone: false
})

export class CancelPurchaseOrderComponent {
  isLoading = false;
  tAmount: any = 0;
  tDiscount: any = 0;
  tExpense: any = 0;
  ...
  async cancel() {
    (await this.purchaseorderService.cancelPurchaseOrder(this.data.element.id)).subscribe({
      next: (data) => {
        if (data == true) {
          this.isLoading = false;
          this.notificationsService.showNotification('Cancel Successfully', 'snack-bar-success');
