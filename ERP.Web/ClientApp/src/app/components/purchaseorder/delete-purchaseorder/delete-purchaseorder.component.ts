...
@Component({
    selector: 'app-delete-purchaseorder',
    templateUrl: './delete-purchaseorder.component.html',
    styleUrl: './delete-purchaseorder.component.css',
    standalone: false
})
...
export class DeletePurchaseOrderComponent {
  ...
  constructor(private dialog: MatDialog, private formBuilder: FormBuilder, private notificationsService: NotificationsService, private purchaseorderService: PurchaseOrderService, private constantService: ConstantService, @Inject(MAT_DIALOG_DATA) public data: { element: any }) { }
  ...
  async delete() {
    (await this.purchaseorderService.deletePurchaseOrder(this.data.element.id)).subscribe({
      next: (data) => {
        if (data == true) {
