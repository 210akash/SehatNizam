...
import { CancelPurchaseOrderComponent } from '../cancel-purchaseorder/cancel-purchaseorder.component';
...
export class PurchaseOrderListComponent {
  @Output() getPurchaseOrderCount: EventEmitter<void> = new EventEmitter<void>();
  PurchaseOrderFilterForm!: FormGroup;
  isLoading = false;
  currentPage = 0;
  pageSizeOptions: number[] = [5, 10, 25, 100];
  displayedColumns: string[] = ['code', 'vendor', 'createdDate', 'createdBy', 'status', 'actions'];
  dataSource: any;
  take = 50;
  pageSize = 0;
  totalRows = 0;
  ...
  async bindData(purchaseorderFilterForm: any, currenttab: number, isFromParent: boolean): Promise<void> {
    ...
    if (currenttab == 0) {
      this.displayedColumns = ['code', 'vendor', 'createdDate', 'createdBy', 'status', 'actions'];
    }
    else if (currenttab == 1) {
      this.displayedColumns = ['code', 'vendor', 'processedDate', 'processedBy', 'status', 'actions'];
    }
    else if (currenttab == 2) {
      this.displayedColumns = ['code', 'vendor', 'approvedDate', 'approvedBy', 'status', 'actions'];
    }
  ...
  pageChanged(event: PageEvent): void {
    ...
    this.bindData(this.PurchaseOrderFilterForm, this.currenttab, false); // Re-fetch data on page change
  }
  ...
  openPurchaseOrderDialog(element: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      this.bindData(this.PurchaseOrderFilterForm, this.currenttab, false);
      this.getPurchaseOrderCount.emit();
    });
  }
  ...
  cancelPurchaseOrderDialog(element: any) {
    const dialogRef = this.dialog.open(CancelPurchaseOrderComponent, {
      panelClass: 'cstm_width_1000',
      maxHeight: '90vh',
      data: {
        element: element,
