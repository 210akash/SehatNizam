...
export class PurchaseOrderTabComponent implements OnInit {
  ...
  constructor(private vendorService: VendorService, private purchaseorderService: PurchaseOrderService, private notificationsService: NotificationsService, private dialog: MatDialog, private constantService: ConstantService, private formBuilder: FormBuilder) { }
  ...
  async changeTab(event: MatTabChangeEvent) {
    ...
    await this.purchaseorderListComponent.bindData(this.lObjLeadsFilterForm, this.selected.value, true);
  }
  ...
  generateRoleWiseTab() {
    ...
    this.statusTab = [
      ...
      { label: 'Canceled', id: 4, pCount: this.CountCanceled, statusId: 5 },
    ];
  }
    ...
    (this.purchaseorderService.getIndentRequestCount(this.lObjLeadsFilterForm)).subscribe(
      {
        next: async (data: any) => {
          ...
          this.CountCanceled = data.item4;
          this.CountIssued = data.item5;
          this.addTab();
        }, error(error: any) {
          console.log(error);
        }
  ...
  onOptionSupplierSelected(event: MatAutocompleteSelectedEvent): void {
    ...
    this.purchaseorderFilterForm.get('vendorId')?.patchValue(selectedValue.id);
    this.purchaseorderFilterForm.get('vendorName')?.patchValue(selectedValue.name);
    this.filterData();
  }
  ...
  onInputCleared(event: Event): void {
    ...
    if (!inputValue.trim()) {
      this.purchaseorderFilterForm.get('vendorId')?.patchValue(null);
      this.filterData();
    }
  }
