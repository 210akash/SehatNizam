...
@Component({
    selector: 'app-delete-salematerial',
    templateUrl: './delete-salematerial.component.html',
    styleUrl: './delete-salematerial.component.css',
    standalone: false
})

export class DeleteSaleMaterialComponent {
  isLoading = false;
  ...
  advFEDAmt!: number;
  constructor(private dialog: MatDialog,private notificationsService: NotificationsService, private salematerialService: SaleMaterialService, @Inject(MAT_DIALOG_DATA) public data: { element: any }) { }
  ngOnInit(): void {
    this.calculateTotal();
  }
  ...
  async delete() {
    (await this.salematerialService.deleteSaleMaterial(this.data.element.id)).subscribe({
      next: (data) => {
        if (data == true) {
