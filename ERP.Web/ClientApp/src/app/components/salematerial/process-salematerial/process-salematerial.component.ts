...
@Component({
    selector: 'app-process-salematerial',
    templateUrl: './process-salematerial.component.html',
    styleUrl: './process-salematerial.component.css',
    standalone: false
})

export class ProcessSaleMaterialComponent {
  isLoading = false;
  ...
  advFEDAmt!: number;
  constructor(private dialog: MatDialog, private notificationsService: NotificationsService, private salematerialService: SaleMaterialService, private constantService: ConstantService, @Inject(MAT_DIALOG_DATA) public data: { element: any }) { }
  ...
  async process() {
    (await this.salematerialService.processSaleMaterial(this.data.element.id)).subscribe({
      next: (data: boolean) => {
        if (data == true) {
