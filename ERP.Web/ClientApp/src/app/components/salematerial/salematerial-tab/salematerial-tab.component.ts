...
import { SaleMaterialListComponent } from '../salematerial-list/salematerial-list.component';

@Component({
  selector: 'app-salematerial-tab',
  templateUrl: './salematerial-tab.component.html',
  styleUrls: ['./salematerial-tab.component.css'],
  standalone: false
})

export class SaleMaterialTabComponent implements OnInit {
  ...
  constructor(private salematerialService: SaleMaterialService, private notificationsService: NotificationsService, private dialog: MatDialog, private constantService: ConstantService, private formBuilder: FormBuilder) { }
  @ViewChild(SaleMaterialListComponent) salematerialListComponent!: SaleMaterialListComponent;
  async ngOnInit() {
    ...
    this.saleMaterialFilterForm.get('fdate')?.patchValue(this.constantService.formatDate(startDate));
    this.saleMaterialFilterForm.get('tdate')?.patchValue(this.constantService.formatDate(endDate));
  ...
  async changeTab(event: MatTabChangeEvent) {
    ...
    await this.salematerialListComponent.bindData(this.lObjLeadsFilterForm, this.selected.value, true);
  }
  generateRoleWiseTab() {
    this.statusTab = null;
  ...
  async fillGridCount(statusId: any) {
    this.isLoading = true;
    let _saleMaterialFilterForm: any = {};
    this.currentUser = JSON.parse(localStorage.getItem('currentUser') ?? '{}');
    this.saleMaterialFilterForm.get("statusId")?.patchValue(statusId);
    _saleMaterialFilterForm = Object.assign(_saleMaterialFilterForm, this.saleMaterialFilterForm.value);
    this.lObjLeadsFilterForm = _saleMaterialFilterForm;
    await this.getSaleMaterialCount();
  }

  async getSaleMaterialCount() {
    (this.salematerialService.getSaleMaterialCount(this.lObjLeadsFilterForm)).subscribe(
      {
        next: async (data: any) => {
  ...
  resetForm() {
    this.saleMaterialFilterForm.reset({
      code: "",
      fdate: new Date(),
      tdate: new Date(),
    });
  ...
  async openSaleMaterialDialog(element: any) {
