...
import { ViewSJVComponent } from '../view-sjv/view-sjv.component';
import { PrintSJVComponent } from '../print-sjv/print-sjv.component';

@Component({
  selector: 'app-sjv-list',
  templateUrl: './sjv-list.component.html',
  styleUrls: ['./sjv-list.component.css'],
  standalone: false
})

export class SJVListComponent {
  [x: string]: any;
  ...
  constructor(
    private sjvService: TransactionService,
  ...
  async bindData(sjvFilterForm: any, currenttab: number, isFromParent: boolean): Promise<void> {
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      this.isLoading = true;
      this.TransactionFilterForm = sjvFilterForm;
      ...
      sjvFilterForm["PagingData"] = pagingData;
      let fdate = new Date(sjvFilterForm.fdate);
      let tdate = new Date(sjvFilterForm.tdate);

      sjvFilterForm['fdate'] = fdate.toLocaleDateString();
      sjvFilterForm['tdate'] = tdate.toLocaleDateString();

      // Call the service method and subscribe with the observer

      (await this.sjvService.getAllTransactions(sjvFilterForm)).subscribe({
        next: (data: any) => {
  ...
  viewTransactionDialog(element: any): void {
    this.dialog.open(ViewSJVComponent, {
  ...
  printTransactionDialog(element: any) {
    const dialogRef = this.dialog.open(PrintSJVComponent, {
