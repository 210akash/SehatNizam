...
export class AddTransactionComponent {
  ...
  ngOnInit(): void {
    this.transactionForm = this.formBuilder.group({
      ...
      voucherTypeId: [1, Validators.required],
      status: [''], // Validation
      remarks: [''], // Validation
  ...
  addTransactionDetail(index: number) {
    const newDetailGroup = this.formBuilder.group({
      ...
      accountId: [null, Validators.required], // Validation
      account: [null, Validators.required], // Validation
      accountName: ['', Validators.required], // Validation
      accountGroupId: [null], // Validation
      accountGroup: [null], // Validation
      accountGroupName: [''], // Validation
      quantity: [0, Validators.required], // Validation
  ...
  getuomValue(index: number): any {
    const detailControl = (
      this.transactionForm.get('transactionDetails') as FormArray
    ).at(index);
  ...
  LoadData(element: any) {
    if (element != null) {
      ...
      const detailsArray = this.transactionForm.get(
        'transactionDetails'
      ) as FormArray;
      ...
      if (element.transactionDetails && element.transactionDetails.length > 0) {
        ...
        element.transactionDetails.forEach((detail: any) => {
          ...
          const accountIdValidator = detail.isGroup
            ? Validators.nullValidator
            : Validators.required;
          const accountGroupIdValidator = detail.isGroup
            ? Validators.required
            : Validators.nullValidator;
          ...
          const detailGroup = this.formBuilder.group({
            ...
            accountId: [
              detail.accountId == 0 ? null : detail.accountId,
              accountIdValidator,
            ], // Use the correct value based on isGroup
            ...
            accountGroupId: [
              detail.accountGroupId == 0 ? null : detail.accountGroupId,
              accountGroupIdValidator,
            ], // Use the correct value for accountGroupId
            accountGroup: [detail.accountGroup ? detail.accountGroup : ''],
            accountGroupName: [
              detail.accountGroup
                ? detail.accountGroup.code + ':' + detail.accountGroup.name
                : '',
              Validators.required,
            ],
            ...
            quantity: [detail.quantity, Validators.required], // Validation
            ...
            isGroup: [detail.isGroup], // Validation
          });
          detailsArray.push(detailGroup);
          this.setvalidators(detail.isGroup, i, true);
          i++;
        });
      }
      ...
      this.data.element.transactionDocuments.forEach((filedata: any) => {
        this.uploadedMedia.push({
          ...
          FileSize:
            this.mediaService.getFileSize(filedata.path) +
            ' ' +
            this.mediaService.getFileSizeUnit(filedata.path),
  ...
  SaveData() {
    ...
    _transactionForm = Object.assign(
      _transactionForm,
  ...
  getTransactionCode() {
    ...
    var VoucherDate = this.transactionForm.get('date')?.value;
    this.transactionService
      .getTransactionCode(VoucherTypeId, VoucherDate)
      .subscribe((data: any) => {
  ...
  getAccountList(event: any) {
    ...
    this.accountService.getAccountByName(filter, accountFlow).subscribe(
      (data: any) => {
  ...
  onOptionSelected(event: MatAutocompleteSelectedEvent, index: number): void {
    ...
    const transactionDetailArray = this.transactionForm.get(
      'transactionDetails'
    ) as FormArray;
    ...
    if (
      ...
      index < 0 ||
      index >= transactionDetailArray.length
    ) {
    ...
    if (duplicateItem) {
      ...
      currentFormGroup.get('accountId')?.patchValue(null);
      currentFormGroup.get('accountName')?.patchValue('');
      currentFormGroup.get('account')?.patchValue(null);
      currentFormGroup.get('accountGroupId')?.patchValue(null);
      currentFormGroup.get('accountGroupName')?.patchValue('');
  ...
  getaccount(itemId: string) {
    return this.accountList.find(
      (option: { id: string }) => option.id === itemId
    );
  }

  getaccountgroup(itemId: string) {
    return this.accountGroupList.find(
      (option: { id: string }) => option.id === itemId
    );
  }
  ...
  displayFn(item: any): string {
    if (item != '') {
      ...
      // If it's just an itemId (number), find the item in the transactionDetails array
        ...
        var selectedItem = this.data.element.transactionDetails.filter(
          (element: any) => {
        ...
        return selectedItem[0]
          ? `${selectedItem[0].item?.code} : ${selectedItem[0].item.name}`
          : '';
      }
  ...
  resetitem(index: number) {
    ...
    this.calculateTotals(); // Recalculate totals after clearing
    return; // Do nothing if the itemId already exists
  }
  ...
  getdepartmentList() {
    ...
    this.departmentService
      .getAllDepartments(_departmentFilter)
      .subscribe((data: any) => {
  ...
  async changeDrCr(type: any, index: number) {
    ...
    if (type == 'dr') {
      ...
      detailFormGroup.get('creditAmount')?.clearValidators();
      detailFormGroup.get('creditAmount')?.updateValueAndValidity();
      detailFormGroup
        .get('creditAmount')
        ?.setValidators([Validators.min(0), Validators.max(0)]);
      detailFormGroup.get('creditAmount')?.updateValueAndValidity();
      detailFormGroup.get('debitAmount')?.clearValidators();
      detailFormGroup
        .get('debitAmount')
        ?.setValidators([Validators.required, Validators.min(1)]);
      detailFormGroup.get('debitAmount')?.updateValueAndValidity();
    } else {
  ...
  async calculateTotals() {
    ...
    (this.transactionDetails.controls as FormGroup[]).forEach(
      (control: FormGroup) => {
  ...
  getaccountSearch(itemId: string) {
    return this.accountSearchList.find(
      (option: { id: string }) => option.id === itemId
    );
  }
  ...
  getcategoryList() {
    ...
    this.categoryService
      .getAllAccountCategorys(_CategoryFilter)
      .subscribe((data: any) => {
  ...
  getsubcategoryList() {
    ...
    var AccountCategoryId =
      this.accountSearchForm.get('accountCategoryId')?.value;
    this.subcategoryService
      .getSubCategoryByCategory(AccountCategoryId)
      .subscribe((data: any) => {
  ...
  getAccountTypeList() {
    ...
    searchForm.get('account')?.patchValue(null);
    searchForm.get('accountNameSearch')?.patchValue('');
    var accountTypeId = this.accountSearchForm.get(
      'accountSubCategoryId'
    )?.value;
    this.accounttypeService
      .getAccounttypeBySubCategory(accountTypeId)
      .subscribe((data: any) => {
  ...
  getAccountSearchList(event: any) {
    ...
    this.accountService
      .getAllAccounts(_AccountFilterForm)
      .subscribe((data: any) => {
        this.accountSearchList = data.item1;
      });
  }

  onOptionSelectedSearch(
    ...
    if (duplicateItem) {
      ...
      searchForm
        .get('accountNameSearch')
        ?.patchValue(selectedValue.code + ' : ' + selectedValue.name);
    }
  }

  onOptionGroupSelected(
    event: MatAutocompleteSelectedEvent,
    index: number
  ): void {
    const selectedValue = event.option.value;
    ...
    const duplicateItem = transactionDetailArray.controls
      ...
      .some((control: AbstractControl) => {
        const formGroup = control as FormGroup; // Cast AbstractControl to FormGroup
        return formGroup.get('accountGroupId')?.value === selectedValue.id; // Check if the itemId already exists
      });
    ...
    if (duplicateItem) {
      ...
      currentFormGroup.get('accountGroupId')?.patchValue(null);
      currentFormGroup.get('accountGroupName')?.patchValue('');
      currentFormGroup.get('accountGroup')?.patchValue(null);
      currentFormGroup.get('quantity')?.patchValue(0);
      currentFormGroup.get('debitAmount')?.patchValue(0);
      currentFormGroup.get('creditAmount')?.patchValue(0);
      // This will clear all the values of the current form group
      ...
      // Get the selected item details from your getaccount method
      const selectedItem = this.getaccountgroup(selectedValue.id);
      if (!selectedItem) {
        console.error('Selected item not found.');
        return;
      }
      ...
      // Patch the values into the form group
      detailFormGroup.get('accountGroupId')?.patchValue(selectedValue.id);
      detailFormGroup
        .get('accountGroupName')
        ?.patchValue(selectedValue.code + ' : ' + selectedValue.name);
      detailFormGroup.get('accountGroup')?.patchValue(selectedValue);
    }
    this.calculateTotals();
  }

  showOptions(event: MatCheckboxChange, index: number): void {
    this.setvalidators(event.checked, index, false);
  }

  setvalidators(checked: boolean, index: number, isFirst: boolean) {
    const transactionDetailArray = this.transactionForm.get(
      'transactionDetails'
    ) as FormArray;
    const currentFormGroup = transactionDetailArray.at(index) as FormGroup;
    if (checked) {
      ...
      currentFormGroup.get('accountGroupId')?.updateValueAndValidity();
      currentFormGroup
        .get('accountGroupName')
        ?.setValidators([Validators.required]);
      currentFormGroup.get('accountGroupName')?.updateValueAndValidity();
      currentFormGroup
        .get('accountGroup')
        ?.setValidators([Validators.required]);
      currentFormGroup.get('accountGroup')?.updateValueAndValidity();
      //Clear Required account
      currentFormGroup.get('accountId')?.clearValidators();
      currentFormGroup.get('accountId')?.updateValueAndValidity();
      currentFormGroup.get('accountName')?.clearValidators();
      currentFormGroup.get('accountName')?.updateValueAndValidity();
      currentFormGroup.get('account')?.clearValidators();
      currentFormGroup.get('account')?.updateValueAndValidity();
    } else {
      // Set Required account
      currentFormGroup.get('accountId')?.setValidators([Validators.required]);
      currentFormGroup.get('accountId')?.updateValueAndValidity();
      currentFormGroup.get('accountName')?.setValidators([Validators.required]);
      currentFormGroup.get('accountName')?.updateValueAndValidity();
      currentFormGroup.get('account')?.setValidators([Validators.required]);
      currentFormGroup.get('account')?.updateValueAndValidity();
      //Clear Required accountGroup
      currentFormGroup.get('accountGroupId')?.clearValidators();
      currentFormGroup.get('accountGroupId')?.updateValueAndValidity();
      currentFormGroup.get('accountGroupName')?.clearValidators();
      currentFormGroup.get('accountGroupName')?.updateValueAndValidity();
      currentFormGroup.get('accountGroup')?.clearValidators();
      currentFormGroup.get('accountGroup')?.updateValueAndValidity();
    }
    if (!isFirst) {
      currentFormGroup.get('id')?.patchValue(0);
  ...
  getAccountGroupList(event: any) {
    ...
    this.accountgroupService
      .getAccountGroupByName(filter, accountFlow)
      .subscribe(
        (data: any) => {
