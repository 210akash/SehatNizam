...
export class TransactionService extends BaseService<any> {
    ...
    constructor(httpClient: HttpClient, private http: HttpClient, private transactionEndPoints: TransactionEndPoints,private constantService: ConstantService ) {
        super(
            httpClient,
            environment.dev_uri
        );
    ...
    getTransactionCode(VoucherTypeId:number,VoucherDate : any) {
        var date  = this.constantService.formatDate(VoucherDate);
        return this.get('?voucherTypeId=' + VoucherTypeId + '&voucherDate=' + date ,this.endPointControllerName + this.transactionEndPoints.getTransactionCode)
            .pipe(map((data: any) => data));
    }
    ...
    rejectTransaction(id: number) {
      return this.get('?id='+id, this.endPointControllerName + this.transactionEndPoints.rejectTransaction)
          .pipe();
  }
